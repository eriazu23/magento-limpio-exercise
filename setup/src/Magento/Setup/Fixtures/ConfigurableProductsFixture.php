...
/**
 * Generate configurable product based on profile configuration
 ...
 * <configurable_products>{product amount}</configurable_products>
 *
 * 2.1 Generate product based on existing attribute set:
 * <configurable_products>
 *     <config>
 *          <attributeSet>{Existing attribute set name}</attributeSet>
 *          <sku>{Configurable sku pattern with %s}</sku>
 *          <product>{Amount of configurable product}</product>
 ...
 * </configurable_products>
 *
 * 2.2 Generate product based on dynamically created attribute set with specified amount of attributes and options
 * <configurable_products>
 *     <config>
 ...
 * </configurable_products>
 *
 * 2.3 Generate product based on dynamically created attribute set with specified configuration per each attribute
 ...
 * If node "assign_entities_to_all_websites" from profile is set to "1" then product will be assigned to all websites
 *
...
class ConfigurableProductsFixture extends Fixture
{
    ...
    private function getDefaultAttributeSetsConfig(array $defaultAttributeSets, $configurableProductsCount)
    {
        ...
        foreach ($defaultAttributeSets as $attributeSetName => $attributeSet) {
            ...
            $configurableConfig[] = [
                ...
                'product' => $productsPerSet[$attributeSetName],
    ...
    {
        return 'Generating configurable product';
    }
    ...
    public function printInfo(OutputInterface $output)
    {
        ...
        $generalAmount = array_sum(array_column($configurableProductConfig, 'product'));

        $output->writeln(sprintf('<info> |- Configurable product: %s</info>', $generalAmount));
    }
    ...
    /**
     * Get configurable product configuration for generate product per attribute set
     *
     * @return array
     * @throws ValidatorException
     ...
     *
     * If amount of configurable product set in profile then return predefined attribute sets
    ...
    /**
     * Get configurable product variations value.
     *
     * @return int
     */
