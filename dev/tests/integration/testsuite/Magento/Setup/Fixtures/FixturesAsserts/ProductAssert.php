 ...
 * Class ProductAssert
 *
 * Class provides assertions about product count and product type
...
class ProductAssert
{
    ...
    /**
     * Performs assertion on generated product count
     * Accepts product sku pattern to calculate count and theirs expected count
     *
    ...
    public function assertProductsCount($skuPattern, $expectedCount)
    {
        ...
        if ($expectedCount !== count($productCollection)) {
            throw new \AssertionError(
                sprintf(
                    'Expected amount of product with sku pattern "%s" not equals actual amount',
                    $skuPattern
                )
            );
        }
