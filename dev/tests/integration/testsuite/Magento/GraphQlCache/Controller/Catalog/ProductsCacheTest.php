...
QUERY;
    ...
    public function testRequestNoTagsForNonExistingProducts(): void
    {
        $query
            = <<<QUERY
            {
           product(filter: {sku: {eq: "simple10"}})
           {
               items {
                   id
                   name
...
QUERY;
    ...
    /**
     * Test response has category tags when product are filtered by category id
     *
    ...
    public function testProductsFilterByCategoryHasCategoryTags(): void
    {
        $query
            = <<<QUERY
        {
           product(filter: {category_id: {eq: "333"}})
           {
               items {
                   name
                   sku
