...
class ProductOtherTest extends ProductTestBase
{
    ...
    /**
     * Test importing product with changed SKU letter case.
     */
    public function testImportWithDifferentSkuCase()
    {
        ...
        $this->assertCount(
            3,
            $productRepository->getList($searchCriteria)->getItems(),
            'Ensures that new product were not created'
        );
        foreach ($updatedPrices as $sku => $expectedPrice) {
            $this->assertEquals(
                $expectedPrice,
                $productRepository->get($sku, false, null, true)->getPrice(),
                'Check that all product were updated'
            );
        }
    }

    /**
     * Checks possibility to double importing product using the same import file.
     *
    ...
    /**
     * Checks that product related links added for all bunches properly after product import
     */
