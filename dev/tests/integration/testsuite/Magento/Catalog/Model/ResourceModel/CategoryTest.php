...
class CategoryTest extends TestCase
{
    ...
    /**
     * Test that adding or removing product in a category should not trigger full reindex in scheduled update mode
     *
    ...
    {
        // product are ordered by entity_id DESC because their positions are same and equal to 0
        ...
        $this->categoryResource->save($category);
        // Indices should not be invalidated when adding/removing/reordering product in a category.
        $categoryProductIndexer = $this->getIndexer(CategoryProductIndexer::INDEXER_ID);
        $this->assertTrue(
            $categoryProductIndexer->isValid(),
            '"Indexed category/product association" indexer should not be invalidated.'
        );
        ...
        // catalog product is not update until partial reindex occurs
