...
class CompareListTest extends GraphQlAbstract
{
    ...
    /**
     * Create compare list with product
     *
    ...
    public function testCreateCompareListWithProducts()
    {
        ...
        $mutation =  <<<MUTATION
mutation{
  createCompareList(input:{product: [{$product1->getId()}, {$product2->getId()}]}){
	 uid
     items {
        product {
            sku
...
mutation{
  removeProductsFromCompareList(input: {uid: "{$uid}", product: [{$product->getId()}]}) {
    uid
    items {
        product {
            sku
...
mutation{
    addProductsToCompareList(input: { uid: "{$uid}", product: [{$product1->getId()}, {$product2->getId()}]}) {
        uid
        item_count
        attributes{code label}
        items {
