...
/**
 * Checks the price range of configurable product with disabled child product.
 */
class ConfigurableProductPriceRangeWithDisabledChildProductsTest extends GraphQlAbstract
{
    ...
    public function testConfigurableProductPriceRangeWithDisabledChildProducts(): void
    {
        ...
        $this->assertEquals(
            $enabledChildPrice,
            $response['product']['items'][0]['price_range']['minimum_price']['final_price']['value']
        );
        $this->assertEquals(
            $enabledChildPrice,
            $response['product']['items'][0]['price_range']['minimum_price']['regular_price']['value']
        );
        $this->assertContainsEquals(
            ['product' => ['sku' => $enabledChildSku]],
            $response['product']['items'][0]['variants']
        );
