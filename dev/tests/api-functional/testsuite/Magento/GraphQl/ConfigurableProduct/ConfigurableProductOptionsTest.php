...
/**
 * Test product query for configurable product options
 */
class ConfigurableProductOptionsTest extends GraphQlAbstract
{
    ...
    public function testQueryConfigurableProductLinks($expectedOptions)
    {
        ...
        $this->assertConfigurableProductOptions($response['product']['items'], $expectedOptions);
    }
    ...
    private function getQuery($configurableProduct)
    {
        return <<<QUERY
{
  product(filter: {name: {match: "$configurableProduct"}}) {
    items {
      id
      sku
