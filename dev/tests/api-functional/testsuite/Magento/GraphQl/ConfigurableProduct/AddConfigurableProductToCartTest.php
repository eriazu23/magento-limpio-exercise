...
class AddConfigurableProductToCartTest extends GraphQlAbstract
{
    ...
    public function testAddConfigurableProductToCart()
    {
        ...
        $product = current($searchResponse['product']['items']);
...
QUERY;
        ...
        $this->assertNotEmpty($response['product']['items'], "No result for product with sku: '{$productSku}'");
        return $response['product']['items'][0]['options'];
    }
}
