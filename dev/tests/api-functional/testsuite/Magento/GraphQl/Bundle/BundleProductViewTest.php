...
QUERY;
        ...
        if ((bool)$bundleProduct->getShipmentType()) {
            $this->assertEquals('SEPARATELY', $response['product']['items'][0]['ship_bundle_items']);
        } else {
            $this->assertEquals('TOGETHER', $response['product']['items'][0]['ship_bundle_items']);
        }
        if ((bool)$bundleProduct->getPriceView()) {
            $this->assertEquals('AS_LOW_AS', $response['product']['items'][0]['price_view']);
        } else {
            $this->assertEquals('PRICE_RANGE', $response['product']['items'][0]['price_view']);
        }
        $this->assertBundleBaseFields($bundleProduct, $response['product']['items'][0]);
        $product = $response['product']['items'][0]['items'][0];
        ...
        $this->assertBundleProductOptions($bundleProduct, $response['product']['items'][0]);
        $this->assertNotEmpty(
            $response['product']['items'][0]['items'],
...
QUERY;
        ...
        $this->assertBundleProductOptions($bundleProduct, $response['product']['items'][0], false);
        $this->assertNotEmpty(
            $response['product']['items'][0]['items'],
            "Precondition failed: 'items' must not be empty"
        );
...
QUERY;
        ...
        $this->assertEquals(
            $maximalPrice,
            $response['product']['items'][0]['price']['maximalPrice']['amount']['value']
        );
    }
