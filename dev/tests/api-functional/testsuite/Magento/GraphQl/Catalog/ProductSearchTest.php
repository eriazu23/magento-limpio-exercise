...
class ProductSearchTest extends GraphQlAbstract
{
    ...
    public function testFilterForNonExistingCategory()
    {
        $query = <<<QUERY
{
  product(filter: {category_id: {eq: "99999999"}}) {
    filters {
      name
    }
  }
...
QUERY;
    ...
    public function testUidAndIdUsageErrorOnProductFilteringCategory()
    {
        ...
        $query = <<<QUERY
{
  product(filter: {category_id: {eq: "99999999"}, category_uid: {eq: "OTk5OTk5OTk="}}) {
    filters {
      name
    }
  }
...
QUERY;
     ...
     *
     * Filter product by an array of skus
...
QUERY;
        ...
        $actualFilters = $response['product']['filters'];
    ...
    /**
     * Layered navigation for Configurable product with out of stock options
     * Two configurable product each having two variations and one of the child product
    ...
    public function testLayeredNavigationForConfigurableProducts()
    {
        ...
        $this->assertEquals(2, $response['product']['total_count']);
        $this->assertNotEmpty($response['product']['aggregations']);
        $this->assertNotEmpty($response['product']['filters'], 'Filters is empty');
        $this->assertCount(
            2,
            $response['product']['aggregations'],
            'Aggregation count does not match'
        );

        // Custom attribute filter layer data
        $this->assertResponseFields(
            $response['product']['aggregations'][1],
            [
...
QUERY;
    ...
    /**
     * Filter product by custom attribute of dropdown type and filterTypeInput eq
     *
...
QUERY;
        ...
        $this->assertEquals(
            3,
            $response['product']['total_count'],
            'Number of product returned is incorrect'
        );
        $this->assertTrue(
            count($response['product']['filters']) > 0,
        ...
        $productItemsInResponse = array_map(null, $response['product']['items'], $filteredProducts);
        for ($itemIndex = 0; $itemIndex < $countOfFilteredProducts; $itemIndex++) {
            $this->assertNotEmpty($productItemsInResponse[$itemIndex]);
            //validate that correct product are returned
            $this->assertResponseFields(
        ...
        $this->assertResponseFields(
            $response['product']['aggregations'][2],
            [
                'attribute_code' => $attribute->getAttributeCode(),
                'count' => 1,
                'label' => $attribute->getDefaultFrontendLabel(),
    ...
    /**
     * Filter product using an array of  multi select custom attributes
     *
...
QUERY;
        ...
        $this->assertEquals(3, $response['product']['total_count']);
        $this->assertNotEmpty($response['product']['filters']);
        $this->assertNotEmpty($response['product']['aggregations']);
        $this->assertCount(2, $response['product']['aggregations']);
    }
...
QUERY;
        $response = $this->graphQlQuery($query);
        //Verify total count of the product returned
        $this->assertEquals(3, $response['product']['total_count']);
        $this->assertArrayHasKey('filters', $response['product']);
        $this->assertCount(3, $response['product']['aggregations']);
        ...
        $layers = array_map(null, $expectedFilterLayers, $response['product']['filters']);
        ...
        foreach ($layers as $layerIndex => $layerFilterData) {
            ...
            $this->assertEquals(
                ...
                $response['product']['filters'][$layerIndex]['name'],
                'Layer name does not match'
            );
            $this->assertEquals(
                $layers[$layerIndex][0]['request_var'],
                $response['product']['filters'][$layerIndex]['request_var'],
        ...
        $this->assertResponseFields(
            $response['product']['aggregations'][0],
            [
        ...
        $this->assertCount(7, $response['product']['aggregations'][1]['options']);
        unset($response['product']['aggregations'][1]['options']);
        $this->assertResponseFields(
            $response['product']['aggregations'][1],
            [
...
QUERY;
        ...
        $this->assertNotEmpty($response['product']['filters'], 'filters is empty');
        $this->assertNotEmpty($response['product']['aggregations'], 'Aggregations should not be empty');
        $this->assertCount(3, $response['product']['aggregations']);

        $actualCategoriesFromResponse = $response['product']['aggregations'][1]['options'];

        //Validate the number of categories/sub-categories that contain the product with the custom attribute
...
QUERY;
        $response = $this->graphQlQuery($query);
        $this->assertEquals(1, $response['product']['total_count'], 'More than 1 product found');
        $this->assertCount(2, $response['product']['aggregations']);
        $this->assertResponseFields(
        ...
        $this->assertEquals('Price', $response['product']['aggregations'][0]['label']);
        $this->assertEquals('Category', $response['product']['aggregations'][1]['label']);
        //Disable the product
...
QUERY;
        ...
        $this->assertEquals(0, $response['product']['total_count'], 'Total count should be zero');
        $this->assertEmpty($response['product']['items']);
        $this->assertEmpty($response['product']['aggregations']);
    }
...
QUERY;
        $response = $this->graphQlQuery($query);
        $this->assertEquals(3, $response['product']['total_count'], 'Total count is incorrect');
        $this->assertCount(2, $response['product']['aggregations']);
    ...
    {
        $this->assertArrayHasKey('filters', $response['product'], 'Product has filters');
        $this->assertIsArray(($response['product']['filters']), 'Product filters is not array');
        $this->assertTrue(count($response['product']['filters']) > 0, 'Product filters is empty');
        foreach ($expectedFilters as $expectedFilter) {
            $found = false;
            foreach ($response['product']['filters'] as $responseFilter) {
...
QUERY;
        ...
        $this->assertArrayHasKey('total_count', $response['product']);
        $this->assertProductItems($filteredProducts, $response);
        $this->assertEquals(4, $response['product']['page_info']['page_size']);
    }
    ...
    public function testSortByPosition()
    {
        ...
        $queryAsc = <<<QUERY
{
  product(filter: {category_id: {eq: "$categoryId"}}, sort: {position: ASC}) {
    total_count
    items {
      sku
      name
...
QUERY;
        ...
        $productsAsc = array_column($resultAsc['product']['items'], 'sku');
        ...
        $queryDesc = <<<QUERY
{
  product(filter: {category_id: {eq: "$categoryId"}}, sort: {position: DESC}) {
    total_count
    items {
      sku
      name
...
QUERY;
        ...
        $productsDesc = array_column($resultDesc['product']['items'], 'sku');
        // position equal and secondary sort by entity_id DESC
        ...
        // Reindex product from the result to invalidate query cache.
...
QUERY;
    ...
    public function testSortByPositionWithMultipleCategories(
        ...
        $query = <<<QUERY
{
  product(filter: {category_id: {in: $filter}}, sort: {position: ASC}) {
    total_count
    items {
      sku
    }
...
QUERY;
        ...
        $this->assertArrayNotHasKey('errors', $resultDesc);
        $this->assertEquals($expectedOrderSku, array_column($resultDesc['product']['items'], 'sku'));
    }
    ...
    /**
     * Test product with the same relevance reverse position with ASC and DESC sorting
     *
     * @magentoApiDataFixture Magento/Catalog/_files/category_with_three_products.php
     */
    ...
    protected function getCategoryFilterRelevanceQuery(int $categoryId, string $direction): string
    {
        $query = <<<QUERY
{
  product(filter: {category_id: {eq: "$categoryId"}}, sort: {relevance: $direction}) {
    total_count
    items {
      sku
      name
...
QUERY;
    ...
    /**
     * Filtering for product and sorting using multiple sort parameters
     *
     * @magentoApiDataFixture Magento/Catalog/_files/category.php
...
QUERY;
        ...
        $this->assertEquals(4, $response['product']['page_info']['page_size']);
        $this->assertEquals(1, $response['product']['page_info']['current_page']);
        $this->assertArrayHasKey('sort_fields', $response['product']);
        $this->assertArrayHasKey('options', $response['product']['sort_fields']);
        $this->assertArrayHasKey('default', $response['product']['sort_fields']);
        $this->assertEquals('position', $response['product']['sort_fields']['default']);
        $this->assertArrayHasKey('value', $response['product']['sort_fields']['options'][0]);
        $this->assertArrayHasKey('label', $response['product']['sort_fields']['options'][0]);
        $this->assertEquals('position', $response['product']['sort_fields']['options'][0]['value']);
    }

    /**
     * Filtering product by fuzzy name match
     *
...
QUERY;
        ...
        $response = $this->graphQlQuery($query);
        $this->assertEquals(2, $response['product']['total_count']);
        $this->assertEquals(['page_size' => 2, 'current_page' => 1], $response['product']['page_info']);
        $this->assertEquals(
            [
        ...
        $this->assertArrayHasKey('aggregations', $response['product']);
        $this->assertCount(2, $response['product']['aggregations']);
        ...
        $this->assertEquals($expectedAggregations, $response['product']['aggregations']);
    }
...
QUERY;
        ...
        $expectedProducts = array_column($response['product']['items'], "name", "sku");
        $this->assertEquals($expectedProducts, $actualProducts);
    }

    /**
     * Filter product by single category
     *
...
QUERY;
        ...
        $this->assertEquals(
            2,
            $response['product']['total_count'],
            'Incorrect count of product returned'
        );
        ...
        $links = array_reverse($links);
        foreach ($response['product']['items'] as $itemIndex => $itemData) {
            $this->assertNotEmpty($itemData);
            $this->assertEquals($response['product']['items'][$itemIndex]['sku'], $links[$itemIndex]->getSku());
            /** @var ProductInterface $product */
            $product = $this->productRepository->get($links[$itemIndex]->getSku());
            $this->assertEquals($response['product']['items'][$itemIndex]['name'], $product->getName());
            $this->assertEquals($response['product']['items'][$itemIndex]['type_id'], $product->getTypeId());
            ...
            $categoryInResponse = array_map(
                null,
                $categoryIds,
                $response['product']['items'][$itemIndex]['categories']
            );
     ...
     *
     * Search for product for a fuzzy match and checks if all matching results returned including
...
QUERY;
        ...
        $this->assertEquals(3, $responseDesc['product']['total_count']);
        $this->assertNotEmpty($responseDesc['product']['filters'], 'Filters should have the Category layer');
        $this->assertEquals(
            'Colorful Category',
            $responseDesc['product']['filters'][0]['filter_items'][0]['label']
        );
        $this->assertCount(2, $responseDesc['product']['aggregations']);
        $expectedProductsInResponse = ['Blue briefs', 'Navy Blue Striped Shoes', 'Grey shorts'];
        $namesDesc = array_column($responseDesc['product']['items'], 'name');
        $this->assertEqualsCanonicalizing($expectedProductsInResponse, $namesDesc);
        $this->assertEquals(
            $namesDesc,
            array_reverse(array_column($responseAsc['product']['items'], 'name'))
        );
    }
...
QUERY;
        ...
        $response = $this->graphQlQuery($query);
        $this->assertEquals(1, $response['product']['total_count']);
        $this->assertProductItems($filteredProducts, $response);
        $this->assertEquals(20, $response['product']['page_info']['page_size']);
        $this->assertEquals(1, $response['product']['page_info']['current_page']);
    }
...
QUERY;
    ...
    /**
     * Filter product purely in a given price range
     *
     * @magentoApiDataFixture Magento/Catalog/_files/category.php
     * @magentoApiDataFixture Magento/Catalog/_files/multiple_mixed_products_2.php
     */
...
QUERY;
        ...
        $this->assertCount(2, $response['product']['filters'], 'Filter count does not match');
        $productCount = count($response['product']['filters']);
        for ($i = 0; $i < $productCount; $i++) {
            $this->assertEquals($filterNames[$i], $response['product']['filters'][$i]['name']);
        }
    }
...
QUERY;
        $response = $this->graphQlQuery($query);
        $this->assertEquals(0, $response['product']['total_count']);
        $this->assertEmpty($response['product']['items'], "No items should be returned.");
    }
...
QUERY;
        ...
        self::assertEquals(
            ...
            $responseObject->getData('product/items/0/sku')
        );
        self::assertEquals(
            'Simple Product Visible and InStock',
            $responseObject->getData('product/items/0/name')
        );
        $this->assertEquals(1, $response['product']['total_count']);
    }
...
QUERY;
    ...
    {
        $productItemsInResponse = array_map(null, $actualResponse['product']['items'], $filteredProducts);
