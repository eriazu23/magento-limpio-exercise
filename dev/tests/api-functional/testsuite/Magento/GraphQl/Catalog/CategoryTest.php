        ...
        $query = <<<QUERY
{
  category(id: {$rootCategoryId}) {
      ...
      children {
        id
        name
        productImagePreview: product(pageSize: 1) {
            items {
                id
                }
            }
...
QUERY;
        $response = $this->graphQlQuery($query);
        $this->assertArrayHasKey('product', $response['category']);
        $this->assertArrayHasKey('total_count', $response['category']['product']);
        $this->assertGreaterThanOrEqual(1, $response['category']['product']['total_count']);
        $this->assertEquals(1, $response['category']['product']['page_info']['current_page']);
        $this->assertEquals(20, $response['category']['product']['page_info']['page_size']);
        $this->assertArrayHasKey('sku', $response['category']['product']['items'][0]);
        $firstProductSku = $response['category']['product']['items'][0]['sku'];
        /**
         * @var ProductRepositoryInterface $productRepository
         */
        $productRepository = ObjectManager::getInstance()->get(ProductRepositoryInterface::class);
        $firstProduct = $productRepository->get($firstProductSku, false, null, true);
        $this->assertBaseFields($firstProduct, $response['category']['product']['items'][0]);
        $this->assertAttributes($response['category']['product']['items'][0]);
    }
