...
QUERY;
        ...
        $this->assertEavAttributes($product, $response['product']['items'][0]);
        $this->assertOptions($product, $response['product']['items'][0]);
        self::assertEquals(
            'Movable Position 2',
            $responseObject->getData('product/items/0/categories/0/name')
        );
        self::assertEquals(
            'Filter category',
            $responseObject->getData('product/items/0/categories/1/name')
        );
        //canonical_url will be null unless the admin setting catalog/seo/product_canonical_tag is turned ON
        self::assertNull($responseObject->getData('product/items/0/canonical_url'));
    }
...
QUERY;
        ...
        $this->assertCount(1, $response['product']['items']);
        $this->assertArrayHasKey(0, $response['product']['items']);
        $this->assertMediaGalleryEntries($product, $response['product']['items'][0]);
    }
    ...
    public function testQueryCustomAttributeField()
    {
        ...
        $query = <<<QUERY
{
    product(filter: {sku: {eq: "{$prductSku}"}})
    {
        items
        {
...
QUERY;
        ...
        $this->assertCount(1, $response['product']['items']);
        $this->assertArrayHasKey(0, $response['product']['items']);
        $this->assertCustomAttribute($response['product']['items'][0]);
    }
...
QUERY;
        ...
        $productRepository = ObjectManager::getInstance()->get(ProductRepositoryInterface::class);
        $product = $productRepository->get($productSku, false, null, true);
        $this->assertNotNull($response['product']['items'][0]['product_links'], "product_links must not be null");
        $this->assertProductLinks($product, $response['product']['items'][0]['product_links'][0]);
    }
    ...
    public function testProductPrices()
    {
        ...
        $query = <<<QUERY
       {
           product(filter: {price: {from: "150.0", to: "250.0"}})
           {
               items {
                   id
                   uid
...
QUERY;
        ...
        self::assertNotNull($response['product']['items'][0]['price'], "price must be not null");
        self::assertCount(2, $response['product']['items']);

        // by default sort order is: "newest id first"
        $this->assertBaseFields($secondProduct, $response['product']['items'][0]);
        $this->assertBaseFields($firstProduct, $response['product']['items'][1]);
    }
...
QUERY;
        $response = $this->graphQlQuery($query);
        $this->assertNotEmpty($response['product']['items'][0]['categories'], "Categories must not be empty");
        /** @var CategoryRepositoryInterface $categoryRepository */
        $categoryRepository = ObjectManager::getInstance()->get(CategoryRepositoryInterface::class);
        $categoryIds  = [3, 4, 5];

        $productItemsInResponse = $response['product']['items'];
