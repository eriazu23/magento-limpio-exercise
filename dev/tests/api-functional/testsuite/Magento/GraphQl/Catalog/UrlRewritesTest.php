...
/**
 * Test of getting URL rewrites data from product
 */
...
QUERY;
        ...
        $this->assertArrayHasKey('url_rewrites', $response['product']['items'][0]);
        $this->assertCount(1, $response['product']['items'][0]['url_rewrites']);

        $this->assertResponseFields(
            $response['product']['items'][0]['url_rewrites'][0],
            [
...
QUERY;
        ...
        $this->assertArrayHasKey('url_rewrites', $response['product']['items'][0]);
        $this->assertCount(1, $response['product']['items'][0]['url_rewrites']);
        $this->assertCount($rewritesCount, $response['product']['items'][0]['url_rewrites']);

        for ($index = 0; $index < $rewritesCount; $index++) {
            $urlRewrite = $rewritesCollection[$index];
            $this->assertResponseFields(
                $response['product']['items'][0]['url_rewrites'][$index],
                [
                    "url" => $urlRewrite->getRequestPath(),
                    "parameters" => $this->getUrlParameters($urlRewrite->getTargetPath())
                ]
