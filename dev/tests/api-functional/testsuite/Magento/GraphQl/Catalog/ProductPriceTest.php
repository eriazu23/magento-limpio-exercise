...
class ProductPriceTest extends GraphQlAbstract
{
    ...
    public function testProductWithSinglePrice()
    {
        ...
        $this->assertNotEmpty($result['product']['items']);
        $product = $result['product']['items'][0];
    ...
    /**
     * Pricing for Simple, Grouped and Configurable product with no special or tier prices configured
     *
    ...
    public function testMultipleProductTypes()
    {
        ...
        $result = $this->graphQlQuery($query);
        $this->assertArrayNotHasKey('errors', $result);
        $this->assertCount(3, $result['product']['items']);
        ...
        foreach ($result['product']['items'] as $product) {
            $this->assertNotEmpty($product['price_range']);
    ...
    /**
     * Simple product with special price and tier price with % discount
     *
    ...
    public function testSimpleProductsWithSpecialPriceAndTierPrice(
        ...
        $result = $this->graphQlQuery($query, [], '', $headerMap);
        $this->assertArrayNotHasKey('errors', $result);
        $this->assertCount(2, $result['product']['items']);
    ...
    /**
     * Check the pricing for a grouped product with simple product having special price set
     *
     * @magentoApiDataFixture Magento/GroupedProduct/_files/product_grouped_with_simple.php
     */
    public function testGroupedProductsWithSpecialPriceAndTierPrices()
    {
        ...
        // update default quantity of each of the associated product to be greater than tier price qty of each of them
        foreach ($groupedProductLinks as $groupedProductLink) {
    ...
    public function testSpecialPriceVisibleIfInDateRange()
    {
        $query = <<<QUERY
{
    product(filter: {sku: {eq: "simple_product"}}) {
        items {
            price_range {
                minimum_price {
                    regular_price {
...
QUERY;
        $result = $this->graphQlQuery($query);
        $productInformation = $result['product']['items'][0];
...
QUERY;
    ...
    /**
     * Get GraphQl query to fetch product by sku
     *
     * @param array $skus
     * @return string
     */
    private function getProductQuery(array $skus): string
    {
        ...
        return <<<QUERY
{
  product(filter: {sku: {in: [$stringSkus]}}, sort: {name: ASC}) {
    items {
      name
      sku
      price_range {
...
QUERY;
    ...
    private function getQueryConfigurableProductAndVariants(array $sku): string
    {
        ...
        return <<<QUERY
{
   product(filter: {sku: {in: [$stringSku]}}, sort: {name: ASC}) {
    items {
      name
      sku
      price_range {
...
QUERY;
    ...
    /**
     * Test product with the same price reverse position with ASC and DESC sorting
     *
    ...
    protected function getCategoryFilterPriceQuery(int $categoryId, string $direction): string
    {
        $query = <<<QUERY
{
  product(filter: {category_id: {eq: "$categoryId"}}, sort: {price: $direction}) {
    total_count
    items {
      sku
      name
