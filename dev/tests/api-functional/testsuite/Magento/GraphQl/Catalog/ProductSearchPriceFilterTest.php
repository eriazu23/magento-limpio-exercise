 ...
 *
 * Preconditions:
 *   Fixture simple product created
 * Steps:
 *   Send request:
 *     query test {
 *       product(search: "Product", filter: {price: {from: "0.01" to: "9.99"}}, sort: {price: ASC}) {
 *         items {
 *           name
 *         }
 *         total_count
...
class ProductSearchPriceFilterTest extends GraphQlAbstract
{
    ...
    public function testProductSearchPriceFilter($priceFilter, string $sort, array $items): void
    {
        ...
        $query = <<<QUERY
{
  product(search: "Product", filter: {price: {{$priceFilter}}}, sort: {{$sort}}) {
    items {
      name
      price {
        minimalPrice {
...
QUERY;
        ...
        self::assertCount(
            $totalCount,
            $response['product']['items'],
            "there are should be $totalCount product in price range $priceFilter"
        );

        // prepare first and last item from response for assertions
        $responseFirstItem = reset($response['product']['items']);
        $responseLastItem = end($response['product']['items']);
