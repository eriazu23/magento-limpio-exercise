...
/**
 * Class to test GraphQl response with grouped product
 */
...
QUERY;
        ...
        $this->assertGroupedProductItems($groupedProduct, $response['product']['items'][0]['items']);
        $this->assertNotEmpty(
            $response['product']['items'][0]['product_links'],
            "Precondition failed: 'Linked product items' must not be empty"
        );
        $this->assertProductLinks($groupedProduct, $response['product']['items'][0]['product_links']);
    }
