...
class PriceTiersTest extends GraphQlAbstract
{
    ...
    public function testAllGroups()
    {
        ...
        $itemTiers = $response['product']['items'][0]['price_tiers'];
...
QUERY;
        ...
        foreach ($response['product']['items'] as $key => $item) {
            if (in_array($item['sku'], $productsWithTierPrices)) {
                $this->assertCount(1, $response['product']['items'][$key]['price_tiers']);
            } else {
                $this->assertCount(0, $response['product']['items'][$key]['price_tiers']);
            }
        }
    }
