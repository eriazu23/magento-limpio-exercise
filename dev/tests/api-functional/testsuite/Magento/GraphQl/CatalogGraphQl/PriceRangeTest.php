...
class PriceRangeTest extends GraphQlAbstract
{
    ...
    public function testMinimalPriceForConfigurableProductWithSpecialPrice() : void
    {
        ...
        $this->assertNotEmpty($responseForFirstWebsite['product']);
        $priceRange = $responseForFirstWebsite['product']['items'][0]['price_range'];
        ...
        $this->assertNotEmpty($responseForSecondWebsite['product']);
        $priceRange = $responseForSecondWebsite['product']['items'][0]['price_range'];
    ...
    public function testCheckIfCatalogRuleIsAppliedForTierPriceForAllGroups(): void
    {
        ...
        $priceRange = $response['product']['items'][0]['price_range'];
        $this->assertEquals(10, $priceRange['minimum_price']['regular_price']['value']);
...
query getProductsBySku {
  product(filter: { sku: { eq: "configurable" } }) {
    items {
      sku
      name
      price_range {
