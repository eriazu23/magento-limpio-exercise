 ...
 * for configurable product returns only visible product.
 */
class ProductSearchTest extends GraphQlAbstract
{
    ...
    public function testCheckIfConfigurableProductVisibilityReturnsExpectedItem(): void
    {
        ...
        $this->assertEquals($productName, $response['product']['items'][0]['name']);
        $this->assertEquals($productSku, $response['product']['items'][0]['sku']);
    }
    ...
    private function getProductSearchQuery(string $productName, string $productSku): string
    {
        return <<<QUERY
{
  product(filter: {sku: {eq: "{$productSku}"}}, search: "$productName", sort: {}, pageSize: 200, currentPage: 1) {
    total_count
    page_info {
        total_pages
        current_page
...
QUERY;
    ...
    public function testSearchSuggestions() : void
    {
        ...
        $this->assertNotEmpty($response['product']['suggestions']);
    }
