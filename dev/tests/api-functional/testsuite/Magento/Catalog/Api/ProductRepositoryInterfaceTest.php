...
class ProductRepositoryInterfaceTest extends WebapiAbstract
{
    ...
    /**
     * Keep in mind: Fixture contains 5 product
     *
     * @return array
     */
    ...
    private function getAttributeOptions($attributeCode)
    {
        $serviceInfo = [
            'rest' => [
                'resourcePath' => '/V1/product/attributes/' . $attributeCode . '/options',
                'httpMethod' => Request::HTTP_METHOD_GET,
            ],
            'soap' => [
    ...
    {
        //Updating our admin user's role to allow saving product but not their design settings.
        /** @var Role $role */
        ...
        $rules->setResources(['Magento_Catalog::product']);
        $rules->saveRel();
        ...
        $rules->setRoleId($role->getId());
        $rules->setResources(['Magento_Catalog::product', 'Magento_Catalog::edit_product_design']);
        $rules->saveRel();
