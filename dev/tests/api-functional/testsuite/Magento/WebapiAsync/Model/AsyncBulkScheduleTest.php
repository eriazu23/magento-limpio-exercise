...
/**
 * Check async request for multiple product creation service,
...
class AsyncBulkScheduleTest extends WebapiAbstract
{
    ...
    const REST_RESOURCE_PATH = '/V1/product';
    const ASYNC_BULK_RESOURCE_PATH = '/async/bulk/V1/product';
    ...
    public function testAsyncScheduleBulkMultipleEntities($products)
    {
        ...
        try {
            ...
            $this->fail("Not all product were created");
        }
    }
    ...
    private function clearProducts()
    {
        ...
        if ($size > 0) {
            throw new Exception(new Phrase("Collection size after clearing the product: %size", ['size' => $size]));
        }
    }
